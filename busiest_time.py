...
Given an array, data, of data points, write a function findBusiestPeriod that returns the time at which the mall reached its
busiest moment last year. The return value is the timestamp, e.g. 1480640292.
...
def busiest(data):
    same_day = lambda a, b: a.year == b.year and a.month == b.month and a.day == b.day

    mx, busiest, cur = 0, 0, 0
    cur_date = datetime.fromtimestamp(data[0][0])
    for time, visitors, isEntering in data:
        date = datetime.fromtimestamp(time)
        if same_day(cur_date, date):
            if isEntering:
                cur += visitors
            else:
                cur -= visitors
